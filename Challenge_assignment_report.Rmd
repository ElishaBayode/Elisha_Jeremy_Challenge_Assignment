...
The suspected source of infection is known for `r signif(100* sum(!is.na(generationTimes))/nrow(transformedData),2)`\% of cases thus far. The mean generation time (for cases where the generation time is known) is `r signif(mean( generationTimes, na.rm=TRUE ), 2)` days, with a standard deviation of `r signif(sd( generationTimes, na.rm=TRUE),2)` days.
